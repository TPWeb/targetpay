...
class Netherland extends Country {
	...
	{
		if($this->amountPerAction == 40 && $this->getMode() == "PC") {
			$this->amountPerAction = 0.40;
			$this->amount = 0.35;
			$this->payout = 0.17; //48,57%
		}
		else if($this->amountPerAction == 45 && $this->getMode() == "PC") {
			$this->amountPerAction = 0.45;
			$this->amount = 0.45;
			$this->payout = 0.25; //55,55%
		}
		else if($this->amountPerAction == 55 && $this->getMode() == "PC") {
			$this->amountPerAction = 0.55;
			$this->amount = 0.45;
			$this->payout = 0.33; //60%
		}
		else if($this->amountPerAction == 60 && $this->getMode() == "PC") {
			$this->amountPerAction = 0.60;
			$this->amount = 0.60;
			$this->payout = 0.37; //61%
		}
		else if($this->amountPerAction == 70 && $this->getMode() == "PC") {
			$this->amountPerAction = 0.70;
			$this->amount = 0.70;
			$this->payout = 0.45; //64
		}
		else if($this->amountPerAction == 90 && $this->getMode() == "PC") {
			$this->amountPerAction = 0.90;
			$this->amount = 0.90;
			$this->payout = 0.61; //0.67
		}
		...
		else if($this->amountPerAction == 115 && $this->getMode() == "PC") {
			$this->amountPerAction = 1.15;
			$this->amount = 1.15;
			$this->payout = 0.79; //68%
		}
		else if($this->amountPerAction == 130 && $this->getMode() == "PC") {
			$this->amountPerAction = 1.30;
			$this->amount = 1.30;
			$this->payout = 0.92; //70%
		}
		else if($this->amountPerAction == 140 && $this->getMode() == "PC") {
			$this->amountPerAction = 1.40;
			$this->amount = 1.40;
			$this->payout = 0.98; //70%
		}
		else if($this->amountPerAction == 35 && $this->getMode() == "PM") {
			$this->amountPerAction = 0.35;
			$this->amount = round($this->duration / 60 * $this->amountPerAction, 2);
			$this->payout = round(0.12 * round($this->amount / $this->amountPerAction), 2);
		}
		else if($this->amountPerAction == 80 && $this->getMode() == "PM") {
			$this->amountPerAction = 0.80;
			$this->amount = round($this->duration / 60 * $this->amountPerAction, 2);
			$this->payout = round(0.56 * round($this->amount / $this->amountPerAction), 2);
		}
		else if($this->amountPerAction == 90 && $this->getMode() == "PM") {
			$this->amountPerAction = 0.90;
			$this->amount = round($this->duration / 60 * $this->amountPerAction, 2);
